...
let isNotEdid s =
    try
        s |> replace "~" "" |> EDID |> ignore
        None
    with
    | e -> Regex(@"\s*\(.*\)").Replace(e.Message, "") |> Some

let s =
    "0x9DE80~test.esp,ActorTypeNPC,-Nazeem , ActorTypeGhost+Vampire,*Draugr , ActorTypeNPC, @Balgruuf the ,cc$e, 0x84E~[full_inu] Queen Marika's Dress.esp"

let s2 =
    ",-Nazeem , ActorTypeGhost+Vampire,*Draugr , ActorTypeNPC, @Balgruuf the ,cc$e, "
...
s
|> getElements (fun s -> s |> regexReplace @"\s+" "")
|> Array.choose isNotEdid
...
let values =
    [| "Abelone"
       ...
       "Alexia Vici"
       "AlexiaVici"
       "DA05Hunter11_04Melee_Ahjisi"
       "DA13encAfflicted"
       "DA13EncAfflicted00Template"
       "DA13LvlAfflictedMelee1HWERefugee"
       "DB02Captive2"
       "dunChillwindAdventurer01"
       "dunChillwindAdventurer02"
       "dunDeepwoodAdventurer1"
       "dunFolgunthurAdventurer1"
       "dunFolgunthurAdventurer2"
       "dunFolgunthurAdventurer3"
       "dunFolgunthurAdventurer4"
       "dunFolgunthurAdventurer5"
       "dunHalldirsDeadBandit03"
       "dunNchuandAlethiusCorpse"
       "dunReachwaterRockAdventurer"
       "dunRebelsCairnAdventurer"
       "dunSkybornAltarAdventurer"
       "Jenssen"
       "MS10Pirate1"
       "TreasCorpseAlchemist"
       "WarehouseNPCWAllAmbush"
       "WEAdventurerTemplate" |]

let alwaysStart = false
let multiIndicator = true
let indicator = "+,-"
...
let (textToMatchStart, textToMatch) =
    ...
    | [] -> 0, txt // Predict only from the start
    | a ->
        match indicators
              ...
              alwaysStart
            with
        | -1, false -> -1, "" // Indicators are required, but none was found
        | -1, true -> 0, txt // Predict also from the start
        | start, _ -> // Indicator was expected and found
            start + 1, txt[start + 1 ..]

// salir si el texto está vacío
...
let match' =
    values
    |> Seq.toArray
    |> Array.Parallel.choose (fun s ->
        if s.Length >= txtLen
           && s[.. txtLen - 1]
               .Equals(textToMatch, StringComparison.InvariantCultureIgnoreCase) then
            Some(s)
        //Some(s, s[txtLen..])
        else
            None)
...
| Some (full) ->
    ...
    replaceAt txt textToMatchStart full
